...
import { RatingFormContainer, RatingFormError, RatingFormFooter, RatingFormFooterButton, RatingFormHeader, RatingFormProfile, RatingFormProfileImageContainer, RatingFormTextarea } from "./styles";
...
export function RatingForm({ bookId, userId, onClose }: RatingFormProps) {
    ...
    const { register, handleSubmit, setValue, formState: { errors } } = useForm<RatingFormData>({
    ...
    return (
        <RatingFormContainer onSubmit={handleSubmit(handleSubmitRating)}>
            <RatingFormHeader>
                <RatingFormProfile>
                    <RatingFormProfileImageContainer>
                        <Image src={session?.user?.image || '/default-avatar.png'} alt="" width={40} height={40} />
                    </RatingFormProfileImageContainer>
            ...
            <RatingFormTextarea {...register('description')} placeholder="Escreva sua avaliação" />
            <div>
                {errors.rate && <RatingFormError>{errors.rate.message}</RatingFormError>}
                {errors.description && <RatingFormError>{errors.description.message}</RatingFormError>}
            </div>
            <RatingFormFooter>
